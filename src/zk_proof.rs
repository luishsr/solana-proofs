...
pub fn generate_block_proof(block_hash: Fr, transaction_hashes: Vec<Fr>) -> (String, Vec<u8>) {
...
pub fn save_vk_to_file(vk_bytes: &[u8]) {
    let file_name = Path::new("proofs/verification_key.json");
    let mut file = File::create(&file_name).expect("Unable to create file");

    file.write_all(vk_bytes).expect("Unable to write data to file");

    println!("Saved verification key to {:?}", file_name);
}
...
fn serialize_vk(vk: &groth16::VerifyingKey<Bls12>) -> Vec<u8> {
